...
class EpisodeReplay:
    ...
    def load_state_dict(self, state):
        self.completed = deque(episode.to(self.storage_device) for episode in state.get("completed", ()))
    ...
    def ready(self, length):
        return any(len(episode) >= length for episode in self.completed) or any(
            len(episode) >= length for episode in self.current or ()
        )
    ...
    def sample_groups(self, batch_size, sequence_length, windows_per_episode=1):
        batch_size = int(batch_size)
        sequence_length = int(sequence_length)
        windows_per_episode = max(1, int(windows_per_episode))
        candidates = self.episodes(sequence_length)
        if not candidates:
            raise RuntimeError(f"No replay episode has length >= {sequence_length}.")

        group_count = math.ceil(batch_size / windows_per_episode)
        ...
        selected = torch.multinomial(
            weights,
            group_count,
            replacement=group_count > len(candidates),
        ...
        for candidate_index in selected.tolist():
            ...
            count = min(windows_per_episode, remaining)
            valid_starts = len(episode) - sequence_length + 1
            if valid_starts >= count:
                starts = torch.randperm(valid_starts, generator=self._generator)[:count]
            else:
                starts = torch.randint(valid_starts, (count,), generator=self._generator)
...
class Buffer(EpisodeReplay):
    ...
    def __init__(self, config):
        ...
        self.windows_per_episode = int(config.windows_per_episode)
        super().__init__(
            config.max_size,
    ...
    def ready(self):
        return super().ready(self.sequence_length)
...
class SequenceBuffer(EpisodeReplay):
    ...
    def state_dict(self):
        return {"obs_keys": self._obs_keys, "replay": super().state_dict()}
