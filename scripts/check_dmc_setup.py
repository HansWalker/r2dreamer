...
def check_mamba3():
    ...
    for name, d_model, config, amp_dtype, batch_size in configs:
        ...
        if not torch.allclose(output.detach().float(), recurrent.detach().float(), rtol=1e-1, atol=1e-1):
        ...
        if not torch.allclose(recurrent.detach().float(), step.float(), rtol=1e-1, atol=1e-1):
        ...
        for recurrent_state, fast_state in zip(recurrent_cache, fast_cache, strict=True):
            if not torch.allclose(recurrent_state.detach().float(), fast_state.float(), rtol=1e-1, atol=1e-1):
