...
def summary(values):
    ...
    return dict(
        zip(
            ("min", "p05", "median", "p95", "max", "mean", "std"),
            [*np.quantile(values, [0, 0.05, 0.5, 0.95, 1]).tolist(), float(values.mean()), float(values.std())],
            strict=True,
        )
    )
...
def scan_numeric(h5, metadata, config, errors, warnings):
            ...
            records[split].append(
                {
                    ...
                    "normalized_reward": reward.mean() / repeat,
                    ...
                    "state_mean": state.mean(0, dtype=np.float64),
                    "state_square": np.square(state.astype(np.float64)).mean(0),
    ...
    for split, rows in records.items():
        if not rows:
            ...
            continue
        weights = np.asarray([row["length"] + 1 for row in rows])
        mean = np.average([row["state_mean"] for row in rows], axis=0, weights=weights)
        variance = np.average([row["state_square"] for row in rows], axis=0, weights=weights) - mean**2
        ...
        stats = {
            ...
            "transitions": sum(row["length"] for row in rows),
            **{
            ...
            "physical_std": np.sqrt(np.maximum(variance, 0)).tolist(),
            "physical_min": np.min([row["state_min"] for row in rows], axis=0).tolist(),
            "physical_max": np.max([row["state_max"] for row in rows], axis=0).tolist(),
...
def main():
    ...
    report = {
        "audit_version": "dmc_dataset_audit_v1",
    ...
    for config in configs:
        ...
        for split, stats in result.get("numeric", {}).get("splits", {}).items():
            if stats["valid_numeric_episodes"]:
                print(
                    f"  {split:7} | episodes={stats['valid_numeric_episodes']:,}"
                    f" | return mean={stats['return']['mean']:.2f}, p05={stats['return']['p05']:.2f}"
                    f" | late_success={stats['late_success_rate']:.1%}"
                )
                if stats["post_context_near_static_fraction"] is not None:
                    print(f"  {split:7} | near_static_after_prefix={stats['post_context_near_static_fraction']:.1%}")
