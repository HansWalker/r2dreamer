...
def migrate(model, family, checkpoint, dataset, windows, context):
    ...
    try:
        ...
        old_scale = model.state_head.training_scale.clone()
        ...
        return {
            ...
            "previous_training_scale": old_scale.cpu().tolist(),
            "training_scale": model.state_head.training_scale.cpu().tolist(),
            "head_optimizer_reset": not torch.equal(old_scale, model.state_head.training_scale),
        }
    finally:
        model.train(was_training)


def online_prefix(config, family, model, envs, env_steps, metrics_path):
    ...
    try:
        ...
        with h5py.File(dataset_path / "data.hdf5", "r") as h5:
            ...
            result["online"] = online_prefix(config, family, model, envs, args.env_steps,
                                             Path(args.result_path).with_name("metrics.jsonl"))
            close_envs(envs)
            envs = None
...
def write_summary(output, results, args):
    ...
    for result in results:
        ...
        lines.append(f"PASS | {name} | {result['online']['updates']} | {result['migration']['max_prediction_difference']:.3g} | "
                     f"{observed[0]:.3g}->{observed[1]:.3g} | {forecast[0]:.3g}->{forecast[1]:.3g} | {duration(result['elapsed_seconds'])}")
    lines.extend(["PASS verifies execution, finite updates, and migration; it does NOT establish long-run learning stability.",
