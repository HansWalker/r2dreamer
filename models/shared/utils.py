"""Small utility functions shared by the model families."""
...
def parse_model_io(config):
    observations = {str(key): tuple(map(int, shape)) for key, shape in config.observations.items()}
    action_shape = tuple(map(int, config.action.shape))
